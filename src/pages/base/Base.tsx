...
const Base = () => {
    ...
    const configKey = type && Object.keys(baseConfigs).includes(type)
        ? (type as keyof typeof baseConfigs)
        : Object.keys(baseConfigs).find(key => baseConfigs[key as keyof typeof baseConfigs]
                                                     .apiUrl.endsWith(`/${type}`));
    //경로오류
    if (!type || !configKey) {
        return (
            <div>
