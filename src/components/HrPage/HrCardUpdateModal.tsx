...
import { useHrCardInfo, useHrCardUpdate } from "../../apis/HrCardService";
...
const HrCardUpdateModal = ({ isOpen, userId = null, onClose }: Props) => {
    ...
    const profilePreviewSrc = selectedProfile?.previewUrl ?? cardData?.profileUrl ?? "";
    const profileFileText = selectedProfile?.fileName ?? (
        cardData?.profileUrl ? "등록된 프로필 사진" : "선택된 파일 없음"
    );
    const profileInitial = form.userName.trim().charAt(0) || "사진";

    useEffect(() => {
        return () => {
            if (selectedProfile?.previewUrl) {
                URL.revokeObjectURL(selectedProfile.previewUrl);
            }
        };
    }, [selectedProfile]);
    ...
    useEffect(() => {
        ...
        setForm((prev) => {
            ...
            if (!prev.gradeName && prev.gradeId) {
            ...
            <div className="hrCardAddModal-panel" onClick={(event) => event.stopPropagation()}>
                ...
                {isLoading ? (
                    ...
                    <form id={formId} className="hrCardAddModal-form" onSubmit={handleSubmit}>
                        <div className="hrCardUpdateModal-topRow">
                            <div className="hrCardUpdateModal-profileCard">
                                <div className="hrCardUpdateModal-profileTitleRow">
                                    <label className="hrCardAddModal-label">프로필</label>
                                </div>

                                <div className="hrCardUpdateModal-profilePreview">
                                    {profilePreviewSrc ? (
                                        <img
                                            src={profilePreviewSrc}
                                            alt={`${form.userName || "직원"} 프로필`}
                                            className="hrCardUpdateModal-profileImage"
                                        />
                                    ) : (
                                        <div
                                            className="hrCardUpdateModal-profilePlaceholder"
                                            aria-hidden="true"
                                        >
                                ...
                                <input
                                    ...
                                    type="file"
                                    accept="image/*"
                                    className="hrCardUpdateModal-fileInput"
                                ...
                                <button
                                    type="button"
                                    className="hrCardUpdateModal-profileButton"
                                ...
                                <span className="hrCardUpdateModal-profileFileName">
                                    {profileFileText}
                                </span>
                            </div>

                            <div className="hrCardUpdateModal-leadingField">
                                ...
                                <div className="hrCardAddModal-row hrCardAddModal-row--optionFields hrCardUpdateModal-row--optionStack hrCardUpdateModal-inlineOptions">
