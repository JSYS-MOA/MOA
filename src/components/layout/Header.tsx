...
const Header = ({userDept, onMenuClick}:HeaderProps) => {
      ...
      <ul className="menu-List">
        {menuData.map((item) => {
            const hasAccess = item.depts === 'all' ||
                (Array.isArray(item.depts) && item.depts.includes(typeof userDept === "string" ? userDept :""));

            if (!hasAccess) return null;
            return(
            (item.depts === 'all' || item.depts.includes(typeof userDept === "string" ? userDept :"")) && (
                <li
                    key={item.id}
                    className="menu-Item"
